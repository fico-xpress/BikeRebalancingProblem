...
void saveTimeToInfoDf(DataFrame& infoDf, TimeDataType start, TimeDataType end, std::string columnName, int NR_STATIONS, int NR_SCENARIOS) {
    ...
    if (!infoDf.hasColumnName(columnName)) {
        ...
        fileName << "./time_data/B=" << NR_STATIONS << "_S=" << NR_SCENARIOS << "_BRP_L_SingleU.csv";
...
private:
    ...
    bool generateOptimalityCut(std::vector<double>& E_t, double& e_t, bool multiCut);
...
void TwoStage_LShapedMethod::runLShapedMethod(int NR_BIKES, bool verbose) {
    ...
    bool masterIsOptimal = this->generateOptimalityCut(E_t, e_t, false);
...
void TwoStage_LShapedMethod::addOptimalityCutToMasterProb(std::vector<double>& E_t, double& e_t) {
    ...
    // masterProb.addIntsolCallback()
}

bool TwoStage_LShapedMethod::generateOptimalityCut(std::vector<double>& E_t, double& e_t, bool multiCut) {
    ...
    for (int s=0; s<NR_SCENARIOS; s++) {
        ...
        std::vector<double> E_t_s = myElementWiseMultiplication(p_s[s], myMultiplyMatrices(std::vector<std::vector<double>>{pi_s_j[s]}, T_s_j_i[s])[0]);
        // Add this scenario's "cut" to overall cut
...
// TODO: prob.postSolve(), prob.keepBasis(), 
...
void BRP_SubProblem::makeInitialSubProbFormulation() {
    ...
    for (int i=0; i<NR_STATIONS; i++) {
        ...
        // if ( -(masterProbSolver->b_i[i] - masterProbSolver->masterSol_x_t[i]) >= masterProbSolver->d_s_i[s][i] ) {
        //     // Station Overflow o
        //     // std::cout << "For station " << i << ": 1st if-statement\n";
        //     // flow = b_i[i] - x[i]         u = 0         o = -d[s][i] - (b_i[i] - x[i])
        //     masterProbSolver->h_s_j[s][0*NR_STATIONS+i]      = masterProbSolver->b_i[i];
        //     masterProbSolver->h_s_j[s][1*NR_STATIONS+i]      = 0.0;
        //     masterProbSolver->h_s_j[s][2*NR_STATIONS+i]      = -masterProbSolver->d_s_i[s][i] - masterProbSolver->b_i[i];

        //     masterProbSolver->T_s_j_i[s][0*NR_STATIONS+i][i] = 1.0;
        //     masterProbSolver->T_s_j_i[s][1*NR_STATIONS+i][i] = 0.0;
        //     masterProbSolver->T_s_j_i[s][2*NR_STATIONS+i][i] = -1.0;
        // }
        // else if ( masterProbSolver->d_s_i[s][i] >= masterProbSolver->masterSol_x_t[i] ) {
        //     // Station Underflow / unmet demand u
        //     // std::cout << "For station " << i << ": 2nd if-statement\n";
        //     // flow = - x[i]         u = d_s_i[s][i] - x[i]          o = 0
        //     masterProbSolver->h_s_j[s][0*NR_STATIONS+i]      = 0.0;
        //     masterProbSolver->h_s_j[s][1*NR_STATIONS+i]      = masterProbSolver->d_s_i[s][i];
        //     masterProbSolver->h_s_j[s][2*NR_STATIONS+i]      = 0.0;

        //     masterProbSolver->T_s_j_i[s][0*NR_STATIONS+i][i] = 1.0;
        //     masterProbSolver->T_s_j_i[s][1*NR_STATIONS+i][i] = 1.0;
        //     masterProbSolver->T_s_j_i[s][2*NR_STATIONS+i][i] = 0.0;
        // }
        // else {
        //     // std::cout << "For station " << i << ": 3rd if-statement\n";
        //     // flow = - d_s_i[i]         u = 0          o = 0
        //     masterProbSolver->h_s_j[s][0*NR_STATIONS+i]      = -masterProbSolver->d_s_i[s][i];
        //     masterProbSolver->h_s_j[s][1*NR_STATIONS+i]      = 0.0;
        //     masterProbSolver->h_s_j[s][2*NR_STATIONS+i]      = 0.0;
    ...
    subProbPtr->addConstraints(NR_STATIONS, [&](int j) {
        ...
        // return (end_of_day_net_recourse_flows[j] == masterProbSolver->h_s_j[s][offset+j] - myScalarProduct(masterProbSolver->T_s_j_i[s][offset+j], masterProbSolver->masterSol_x_t));
...
int main() {
    try {
        ...
        std::vector<std::vector<std::vector<double>>> d_s_ij = {{{ 0,   3,   2 }, { 7,  0,  10 }, { 3,  2, 0}},
                                                                {{ 0,   5,   6 }, { 2,  0,   2 }, { 7,  2, 0}}};

        int NR_STATIONS = d_s_ij[0].size();
        int NR_SCENARIOS = d_s_ij.size();
        std::vector<std::vector<double>> d_s_i = std::vector<std::vector<double>>(NR_SCENARIOS, std::vector<double>(NR_STATIONS, 0));
        for (int s=0; s<NR_SCENARIOS; s++) {
            for (int i=0; i<NR_STATIONS; i++) {
                for (int j=0; j<NR_STATIONS; j++) {
